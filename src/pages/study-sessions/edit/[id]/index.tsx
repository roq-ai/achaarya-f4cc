...
import { getStudySessionById, updateStudySessionById } from 'apiSdk/study-sessions';
...
function StudySessionEditPage() {
  ...
  const handleSubmit = async (values: StudySessionInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateStudySessionById(id, values);
      mutate(updated);
      resetForm();
