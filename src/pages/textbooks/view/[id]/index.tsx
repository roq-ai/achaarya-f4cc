...
import { getTextbookById } from 'apiSdk/textbooks';
import { TextbookInterface } from 'interfaces/textbook';
import { ChapterListPage } from 'pages/chapters';
import { StudySessionListPage } from 'pages/study-sessions';

function TextbookViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<TextbookInterface>(
    () => (id ? `/textbooks/${id}` : null),
    () =>
      getTextbookById(id, {
        relations: ['provider'],
      }),
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('textbook', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/textbooks/edit/${id}`} passHref legacyBehavior>
                    <Button
                ...
                <FormListItem
                  ...
                  text={
                    data?.publication_year
                      ? format(parseISO(data?.publication_year as unknown as string), 'dd-MM-yyyy')
                      : ''
                  }
                />
