...
import { getTextbookById, updateTextbookById } from 'apiSdk/textbooks';
...
function TextbookEditPage() {
  ...
  const handleSubmit = async (values: TextbookInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateTextbookById(id, values);
      mutate(updated);
      resetForm();
