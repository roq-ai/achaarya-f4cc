...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const allowed = await prisma.chapter
    .withAuthorization({
      roqUserId,
  ...
  async function getChapterById() {
    const data = await prisma.chapter.findFirst(convertQueryToPrismaUtil(req.query, 'chapter'));
    return res.status(200).json(data);
  }

  async function updateChapterById() {
    await chapterValidationSchema.validate(req.body);
